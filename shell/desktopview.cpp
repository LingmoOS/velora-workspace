...
void DesktopView::slotContainmentChanged()
{
    ...
    if (m_containment) {
        ...
        QAction *desktopRefresh = new QAction(QIcon::fromTheme(QStringLiteral("view-refresh")), i18n("Refresh"), m_containment);
        m_containment->setInternalAction(QStringLiteral("desktop refresh"), desktopRefresh);
        connect(desktopRefresh, &QAction::triggered, this, [this]() {
            auto *sc = qobject_cast<ShellCorona *>(m_containment->corona());
            if (sc) {
                sc->refreshCurrentShell();
            } else {
                QDBusMessage msg = QDBusMessage::createMethodCall(QStringLiteral("org.kde.PlasmaShell"),
                                                                  QStringLiteral("/PlasmaShell"),
                                                                  QString(),
                                                                  QStringLiteral("refreshCurrentShell"));
